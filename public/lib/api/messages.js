...
module.exports=async(req,res)=>{try{
 ...
 const id=String(req.query?.conversation_id||req.body?.conversationId||'').trim();
 ...
 if(req.method==='GET'){
   const rows=await sbreq(`/rest/v1/messages?conversation_id=eq.${esc(id)}&select=*&order=created_at.asc&limit=5000`);
   return json(res,200,{messages:rows,conversation:c});
 }
 if(req.method==='POST'){
