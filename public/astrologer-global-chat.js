...
(function(){
  ...
  function ensureUI(){if(document.getElementById('bgAstroRequestLayer'))return;
    const css=document.createElement('style');css.id='bgAstroRequestStyle';css.textContent=`
      ...
      #bgAstroRequestBox .bg-body{padding:16px}.bg-req-name{font-size:19px;font-weight:900}.bg-req-meta{color:#bfc0c5;font-size:13px;margin-top:4px}.bg-req-timer{font-size:24px;font-weight:900;margin:13px 0}.bg-req-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px}.bg-req-actions button{border:0;border-radius:11px;padding:12px;font-weight:900;cursor:pointer}.bg-accept{background:#42d466;color:#07140a}.bg-reject{background:#ff4652;color:#fff}.bg-req-status{margin-top:9px;color:#aaa;font-size:12px;min-height:16px}
    ...
    const layer=document.createElement('div');layer.id='bgAstroRequestLayer';layer.innerHTML=`<div id="bgAstroRequestBox"><div class="bg-head"><span>🔔 New Chat Request</span><span id="bgReqCount"></span></div><div class="bg-body"><div class="bg-req-name" id="bgReqName">New user</div><div class="bg-req-meta" id="bgReqMeta">Chat request</div><div class="bg-req-timer" id="bgReqTimer">02:00</div><div class="bg-req-actions"><button class="bg-accept" id="bgReqAccept">✓ Accept</button><button class="bg-reject" id="bgReqReject">✕ Reject</button></div><div class="bg-req-status" id="bgReqStatus"></div></div></div>`;
  ...
  function show(c){current=c;ensureUI();const box=document.getElementById('bgAstroRequestLayer');box.style.display='flex';document.getElementById('bgAstroBell').style.display='none';document.getElementById('bgReqName').textContent=c.user?.full_name||'New User';document.getElementById('bgReqMeta').textContent='Chat request · '+(c.channel||'chat');document.getElementById('bgReqStatus').textContent='';updateTimer();}
  ...
  async function respond(action){if(!current)return;const id=current.id;const btns=[document.getElementById('bgReqAccept'),document.getElementById('bgReqReject')];btns.forEach(b=>b.disabled=true);document.getElementById('bgReqStatus').textContent=action==='accept'?'Accepting…':'Rejecting…';try{const x=await api('/api/astrologer-requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,conversationId:id})});markSeen(id);hide();if(action==='accept'){const cid=x?.conversation?.id||id;location.href='/chat.html?conversation_id='+encodeURIComponent(cid)}else poll()}catch(e){document.getElementById('bgReqStatus').textContent=e.message;btns.forEach(b=>b.disabled=false);setTimeout(poll,1000)}}
