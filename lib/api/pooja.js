...
module.exports=async(req,res)=>{try{
 if(req.method==='GET'){
  ...
  if(u.account_type==='astrologer'){
   if(String(req.query?.bookings||'')==='1'){const rows=await sbreq(`/rest/v1/pooja_bookings?astrologer_id=eq.${esc(u.id)}&select=*&order=created_at.desc&limit=500`);const ids=[...new Set(rows.map(x=>x.pooja_id).filter(Boolean))];const ps=ids.length?await sbreq(`/rest/v1/poojas?id=in.(${ids.map(esc).join(',')})&select=id,name,price,image_url`):[];const pm=new Map(ps.map(x=>[String(x.id),x]));return json(res,200,{bookings:rows.map(x=>({...x,pooja:pm.get(String(x.pooja_id))||null,pooja_name:x.pooja_name||pm.get(String(x.pooja_id))?.name||''}))});}
  ...
  if(u.account_type==='user'){
   if(String(req.query?.bookings||'')==='1'){const rows=await sbreq(`/rest/v1/pooja_bookings?user_id=eq.${esc(u.id)}&select=*&order=created_at.desc&limit=500`);const ids=[...new Set(rows.map(x=>x.pooja_id).filter(Boolean))];const ps=ids.length?await sbreq(`/rest/v1/poojas?id=in.(${ids.map(esc).join(',')})&select=id,name,price,image_url`):[];const pm=new Map(ps.map(x=>[String(x.id),x]));return json(res,200,{bookings:rows.map(x=>({...x,pooja:pm.get(String(x.pooja_id))||null,pooja_name:x.pooja_name||pm.get(String(x.pooja_id))?.name||''}))});}
 ...
 if(req.method==='POST'){
  ...
  if(b.action==='book'){
   ...
   const rows=await sbreq('/rest/v1/pooja_bookings',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(row)});return json(res,201,{booking:rows[0]||row});
  }
  ...
  if(b.action==='booking'){
   ...
   const rows=await sbreq(`/rest/v1/pooja_bookings?id=eq.${esc(id)}&astrologer_id=eq.${esc(u.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status,astrologer_note:String(b.note||''),updated_at:new Date().toISOString()})});return json(res,200,{booking:rows[0]||null});
  }
