...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='POST'){
  ...
  if(b.action==='book'){
   ...
   const me=await profile(u.id,'user');const price=Math.max(0,Number(p.price||0));const row={user_id:u.id,astrologer_id:p.astrologer_id,pooja_id:p.id,user_name:String(me?.full_name||''),phone:String(me?.phone||''),preferred_date:String(b.preferredDate||''),preferred_time:String(b.preferredTime||''),notes:String(b.notes||''),amount:price,status:'pending',wallet_debited:false,wallet_refunded:false};
   if(!row.preferred_date||!row.preferred_time)return json(res,400,{error:'Preferred date and time are required'});
   if(price>0){const wr=await sbreq(`/rest/v1/wallets?user_id=eq.${esc(u.id)}&select=*&limit=1`);let w=wr[0];if(!w){const cr=await sbreq('/rest/v1/wallets',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({user_id:u.id,balance:0})});w=cr[0];}const bal=Number(w?.balance||0);if(bal<price)return json(res,402,{error:`Pooja booking के लिए wallet में कम से कम ₹${price.toFixed(2)} होना चाहिए। अभी ₹${bal.toFixed(2)} है। पहले Wallet Recharge करें।`,required:price,balance:bal});await sbreq(`/rest/v1/wallets?id=eq.${esc(w.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({balance:Math.round((bal-price)*100)/100,updated_at:new Date().toISOString()})});row.wallet_debited=true;}
   try{
    const rows=await sbreq('/rest/v1/pooja_bookings',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(row)});
    if(row.wallet_debited)await sbreq('/rest/v1/wallet_transactions',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:u.id,amount:price,type:'debit',source:'pooja',description:`Pooja booking — ${String(p.name||'Pooja')}`})});
  ...
  if(b.action==='booking'){
   ...
   try{
    const current=(await sbreq(`/rest/v1/pooja_bookings?id=eq.${esc(id)}&astrologer_id=eq.${esc(u.id)}&select=*&limit=1`))[0];if(!current)return json(res,404,{error:'Booking not found'});
    const refund=(status==='rejected'||status==='cancelled')&&current.wallet_debited===true&&current.wallet_refunded!==true;
    const patch={status,astrologer_note:String(b.note||''),updated_at:new Date().toISOString()};if(refund)patch.wallet_refunded=true;
    const rows=await sbreq(`/rest/v1/pooja_bookings?id=eq.${esc(id)}&astrologer_id=eq.${esc(u.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(patch)});
    if(refund){const amount=Number(current.amount||current.price||0);if(amount>0){const wr=await sbreq(`/rest/v1/wallets?user_id=eq.${esc(current.user_id)}&select=*&limit=1`);const w=wr[0];if(w){const next=Math.round((Number(w.balance||0)+amount)*100)/100;await sbreq(`/rest/v1/wallets?id=eq.${esc(w.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({balance:next,updated_at:new Date().toISOString()})});await sbreq('/rest/v1/wallet_transactions',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:current.user_id,amount,type:'credit',source:'pooja_refund',description:`Pooja booking refund — ${String(current.pooja_name||'Pooja')}`})});}}}
    return json(res,200,{booking:normalizeBooking(rows[0]||null),refunded:refund});
   }catch(e){
