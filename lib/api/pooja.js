...
module.exports=async(req,res)=>{try{
 if(req.method==='GET'){
  ...
  if(u.account_type==='astrologer'){
   if(String(req.query?.bookings||'')==='1'){const rows=await sbreq(`/rest/v1/pooja_bookings?astrologer_id=eq.${esc(u.id)}&select=*&order=created_at.desc&limit=500`);return json(res,200,{bookings:rows});}
  ...
  if(u.account_type==='user'){
   if(String(req.query?.bookings||'')==='1'){const rows=await sbreq(`/rest/v1/pooja_bookings?user_id=eq.${esc(u.id)}&select=*&order=created_at.desc&limit=500`);return json(res,200,{bookings:rows});}
