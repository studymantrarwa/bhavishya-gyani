...
function razor(){const id=process.env.RAZORPAY_KEY_ID||'',secret=process.env.RAZORPAY_KEY_SECRET||'';if(!id||!secret)throw new Error('Razorpay keys are not configured. Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in Vercel Environment Variables.');return {id,secret}}
async function rz(path,options={}){const r=razor();const headers={'Content-Type':'application/json',Authorization:'Basic '+Buffer.from(r.id+':'+r.secret).toString('base64'),...(options.headers||{})};const x=await fetch('https://api.razorpay.com/v1'+path,{...options,headers});const t=await x.text();let d;try{d=t?JSON.parse(t):{} }catch{d={message:t}}if(!x.ok)throw new Error(d.error?.description||d.message||`Razorpay ${x.status}`);return d}
