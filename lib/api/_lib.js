...
function makeToken(user){const payload={id:user.id,email:user.email||'',account_type:user.account_type,exp:Date.now()+7*24*60*60*1000};const body=b64(JSON.stringify(payload));return body+'.'+sign(body)}
function verifyToken(token){try{const [body,sig]=String(token||'').split('.');if(!body||!sig||!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(sign(body))))return null;const p=JSON.parse(Buffer.from(body,'base64url').toString('utf8'));if(!p.id||!p.account_type||p.exp<Date.now())return null;return p}catch{return null}}
