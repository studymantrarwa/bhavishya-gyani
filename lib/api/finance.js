...
async function chargeConversation(c){
 ...
 const commPct=await commissionPercent();const commission=Math.round(gross*commPct)/100;const netAmount=Math.max(0,Math.round((gross-commission)*100)/100);
 let payment=null;
 if(paidAmount>0){
 ...
 const earning=(await sbreq('/rest/v1/astrologer_earnings',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({astrologer_id:c.astrologer_id,amount:netAmount,gross_amount:gross,commission_amount:commission,net_amount:netAmount,category:'chat',conversation_id:c.id,earned_at:new Date().toISOString()})}))[0];
 return {charged:true,minutes,rate,gross,commission,net:netAmount,free_minutes_used:free.used,paid_minutes:paidMinutes,wallet_amount:paidAmount,earning,payment};
}
module.exports={commissionPercent,chargeConversation,wallet,grantFreeMinutes,applyActiveChatOffers,userFree};
