...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'){
  ...
  if(requestedId && requestedId!==u.id){
   ...
   const ps=await sbreq(`/rest/v1/profiles?id=eq.${encodeURIComponent(target)}&select=id,full_name,email,phone,avatar_url,blocked,role`);
   ...
   return json(res,200,{astrologer:{...a,full_name:profileRow?.full_name||'Astrologer',email:profileRow?.email||'',phone:profileRow?.phone||'',avatar_url:a.avatar_url||profileRow?.avatar_url||null,education:a.education||app[0]?.education||''}});
  }
