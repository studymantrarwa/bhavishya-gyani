...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'){
  ...
  if(requestedId && requestedId!==u.id){
   ...
   const online=!!a.online&&!!a.last_seen&&(Date.now()-new Date(a.last_seen).getTime()<=75000);
   const reviews=await sbreq(`/rest/v1/reviews?astrologer_id=eq.${encodeURIComponent(target)}&moderation_status=eq.approved&select=id,rating,review,created_at,user:profiles!reviews_user_id_fkey(full_name,avatar_url)&order=created_at.desc&limit=50`).catch(()=>[]);
   return json(res,200,{astrologer:{...a,online,full_name:profileRow?.full_name||'Astrologer',avatar_url:a.avatar_url||profileRow?.avatar_url||null,education:a.education||app[0]?.education||'',reviews,followers:Number(a.followers||0)}});
  }
