 ...
 if(action==='request'||action==='call'){
  ...
  if(action==='request'){
   ...
   if(aid){
    ...
    const body={user_id:u.id,astrologer_id:aid,admin_id:null,status:'requested',channel:'chat',requested_at:new Date().toISOString(),fee_snapshot:Number(a.fee||0),discount_snapshot:Number(a.discount||0)};
