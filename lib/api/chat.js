...
async function kundliForUser(kid,userId){
 ...
 if(!k)return null;if(k.user_id!==userId)return null;return k;
}
async function savePrechat(c,k,u){
 if(!c||!k)return null;
 const body=`__BG_CHAT_INTAKE__\nनाम: ${k.name||''}\nजन्म तारीख: ${k.dob||''}\nजन्म समय: ${k.birth_time||''}\nजन्म स्थान: ${k.place||''}`;
 const payload={conversation_id:c.id,sender_id:u.id,body,kundali_id:k.id};
...
function prechatData(rows){
 ...
 if(!m)return null;const lines=String(m.body).split('\n'),o={kundali_id:m.kundali_id||null};for(const line of lines.slice(1)){const i=line.indexOf(':');if(i<0)continue;const k=line.slice(0,i).trim(),v=line.slice(i+1).trim();if(k==='नाम')o.name=v;if(k==='जन्म तारीख')o.dob=v;if(k==='जन्म समय')o.birth_time=v;if(k==='जन्म स्थान')o.place=v;}return o;
}
...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'){
  ...
  if(id){const c=await expire(await getConv(id));if(!allowed(u,p,c))return json(res,403,{error:'Forbidden'});const allMsgs=await sbreq(`/rest/v1/messages?conversation_id=eq.${esc(id)}&select=*&order=created_at.asc&limit=5000`);const messages=c.status==='accepted'||c.status==='closed'?allMsgs:[];return json(res,200,{conversation:await decorate(c),messages,prechat:prechatData(allMsgs)});}
 ...
 if(action==='request'||action==='call'){
  ...
  const channel=action==='call'?'call':'chat',aid=String(b.astrologerId||'').trim(),kundaliId=String(b.kundaliId||'').trim();
  if(aid){
   if(channel==='chat'&&!kundaliId)return json(res,400,{error:'Chat start se pehle Kundli/birth details required'});
   const selectedKundli=channel==='chat'?await kundliForUser(kundaliId,u.id):null;if(channel==='chat'&&!selectedKundli)return json(res,403,{error:'Selected Kundli is not valid for this user'});
   ...
   if(old&&REOPENABLE.includes(old.status)){const reopened=await reopen(old,a);if(channel==='chat')await savePrechat(reopened,selectedKundli,u);return json(res,200,{conversation:await decorate(reopened),reopened:true});}
   ...
   const rows=await sbreq('/rest/v1/conversations',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});const created=rows[0];if(channel==='chat')await savePrechat(created,selectedKundli,u);return json(res,201,{conversation:await decorate(created)});
  }
