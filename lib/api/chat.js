...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='POST'){
   ...
   if(action==='request'||action==='call'){
     ...
     if(channel==='chat' && (!a.online || a.chat_enabled===false))return json(res,409,{error:'Chat is currently OFF or astrologer is offline'});
     if(channel==='call' && (!a.online || a.call_enabled===false))return json(res,409,{error:'Call is currently OFF or astrologer is offline'});
