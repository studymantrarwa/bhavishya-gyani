...
async function listFor(u,p){
 ...
 else q=`/rest/v1/conversations?or=(user_id.eq.${esc(u.id)},astrologer_id.eq.${esc(u.id)},admin_id.eq.${esc(u.id)})&select=*&order=last_message_at.desc.nullslast,created_at.desc&limit=500`;
...
async function messagesFor(id){return await sbreq(`/rest/v1/messages?conversation_id=eq.${esc(id)}&select=*&order=created_at.asc&limit=5000`)}
module.exports=async(req,res)=>{try{
 ...
 if(action==='request'||action==='call'){
  ...
  if(aid){
   ...
   let kundli=null;if(channel==='chat'){const kid=String(b.kundaliId||'').trim();if(!kid)return json(res,400,{error:'Kundli selection is required before chat request'});kundli=await kundliForUser(kid,u.id);if(!kundli)return json(res,403,{error:'Selected Kundli does not belong to this account'});}
   const oldRows=await sbreq(`/rest/v1/conversations?user_id=eq.${esc(u.id)}&astrologer_id=eq.${esc(aid)}&channel=eq.${esc(channel)}&select=*&order=created_at.desc&limit=1`);
   ...
   const rows=await sbreq('/rest/v1/conversations',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});const c=rows[0];if(kundli)await savePrechat(c,kundli,u);return json(res,201,{conversation:await decorate(c)});
  }
 ...
 if(action==='close'){
  ...
  const rows=await sbreq(`/rest/v1/conversations?id=eq.${esc(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:'closed',closed_at:new Date().toISOString(),retention_until:p?.role==='user'?null:new Date(now()+172800000).toISOString()})});
