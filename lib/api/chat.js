...
const {wallet,chargeConversation,applyActiveChatOffers,userFree,chatBudget,effectiveRate}=require('./finance');
...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'){
  ...
  if(id){
   ...
   if(String(req.query?.action||'')==='timer'){
    if(c.channel!=='chat'||c.status!=='accepted'||!c.accepted_at)return json(res,200,{active:false,remaining_seconds:0});
    const started=new Date(c.chat_started_at||c.accepted_at).getTime();
    const budget=Math.max(0,Number(c.chat_budget_seconds||0));
    const elapsed=Math.max(0,Math.floor((now()-started)/1000));
    const remaining=Math.max(0,budget-elapsed);
    if(remaining<=0){
      const closedAt=new Date().toISOString();
      const bill=await chargeConversation({...c,closed_at:closedAt});
      if(bill.insufficient)return json(res,200,{active:true,remaining_seconds:0,auto_end_failed:true,billing:bill});
      const rows=await sbreq(`/rest/v1/conversations?id=eq.${esc(id)}&status=eq.accepted`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:'closed',closed_at:closedAt,retention_until:p?.role==='user'?null:new Date(now()+172800000).toISOString(),billed_seconds:bill.minutes?Math.min(budget,bill.minutes*60):budget,billed_amount:Number(bill.wallet_amount||0)})});
      const updated=rows[0]||{...c,status:'closed',closed_at:closedAt};
      return json(res,200,{active:false,remaining_seconds:0,auto_ended:true,conversation:await decorate(updated),billing:bill});
    }
    return json(res,200,{active:true,remaining_seconds:remaining,budget_seconds:budget,rate:Number(c.fee_snapshot||0),started_at:c.chat_started_at||c.accepted_at});
   }
   const all=await messagesFor(id);const accepted=HISTORY.includes(c.status)||c.status==='accepted';
   let chatEarning=null;
   if(c.status==='closed'&&p?.role==='astrologer'&&c.astrologer_id===u.id){
    const er=await sbreq(`/rest/v1/astrologer_earnings?conversation_id=eq.${esc(c.id)}&select=net_amount,amount,gross_amount,commission_amount,earned_at&limit=1`);
    if(er[0]) chatEarning={amount:Number(er[0].net_amount??er[0].amount??0),earned_at:er[0].earned_at||null};
   }
   return json(res,200,{conversation:await decorate(c),messages:accepted?all:[],prechat:prechatData(all),chatEarning});
  }
 ...
 if(action==='request'||action==='call'){
  ...
  if(action==='request'){
   ...
   if(aid){
    ...
    const baseRate=Math.max(0,Number(a.fee_per_minute??a.fee??0));
    const discount=[0,25,50,75].includes(Number(a.discount))?Number(a.discount):0;
    const rate=effectiveRate(baseRate,discount);
    if(rate<=0)return json(res,409,{error:'इस astrologer की Chat price अभी Admin ने set नहीं की है। पहले price configure करें।'});
    if(rate>0){
    ...
    const body={user_id:u.id,astrologer_id:aid,admin_id:null,status:'requested',channel:'chat',requested_at:new Date().toISOString(),fee_snapshot:rate,discount_snapshot:discount,billed_seconds:0,billed_amount:0};
 ...
 if(action==='user-confirm'){
  ...
  await applyActiveChatOffers(u.id);
  const budget=await chatBudget(u.id,Number(c.fee_snapshot||0));
  if(Number(c.fee_snapshot||0)<=0)return json(res,409,{error:'Astrologer chat price is not configured'});
  if(budget.total_seconds<60)return json(res,402,{error:`Chat शुरू करने के लिए कम से कम 1 मिनट का balance/free minute चाहिए। Wallet ₹${budget.wallet_balance.toFixed(2)}, Free ${budget.free_minutes} min`,free_minutes:budget.free_minutes,balance:budget.wallet_balance});
  const startAt=new Date().toISOString();
  const rows=await sbreq(`/rest/v1/conversations?id=eq.${esc(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:'accepted',user_confirmed_at:startAt,accepted_at:startAt,chat_started_at:startAt,chat_budget_seconds:budget.total_seconds,billed_seconds:0,billed_amount:0,user_confirm_response_seconds:sec})});return json(res,200,{conversation:await decorate(rows[0])});
 }
 if(action==='close'){
  if(!['user','astrologer','admin'].includes(p?.role))return json(res,403,{error:'Not allowed'});
  const closedAt=new Date().toISOString();
  let bill=null;
  if(c.channel==='chat'&&c.accepted_at&&c.astrologer_id){
   bill=await chargeConversation({...c,closed_at:closedAt});
   if(bill.insufficient)return json(res,402,{error:`Chat charge ₹${bill.required.toFixed(2)} के लिए wallet balance ₹${bill.balance.toFixed(2)} है। पहले wallet recharge करें।`,billing:bill});
  }
  const patch={status:'closed',closed_at:closedAt,retention_until:p?.role==='user'?null:new Date(now()+172800000).toISOString()};
  if(bill){patch.billed_seconds=Math.min(Number(c.chat_budget_seconds||0)||bill.minutes*60,Math.max(0,Math.round((new Date(closedAt).getTime()-new Date(c.accepted_at).getTime())/1000)));patch.billed_amount=Number(bill.wallet_amount||0)}
  const rows=await sbreq(`/rest/v1/conversations?id=eq.${esc(id)}&status=eq.${esc(c.status)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(patch)});
  const updated=rows[0]||{...c,...patch};
  return json(res,200,{conversation:await decorate(updated),billing:bill});
 }
