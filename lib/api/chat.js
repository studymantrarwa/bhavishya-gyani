...
const {wallet,chargeConversation}=require('./finance');
 ...
 if(action==='request'||action==='call'){
  ...
  if(action==='request'){
   ...
   if(aid){
    ...
    const rate=Math.max(0,Number(a.fee_per_minute??a.fee??0)); if(rate>0){const w=await wallet(u.id);if(Number(w.balance||0)<rate)return json(res,402,{error:`Wallet balance कम है। कम से कम ₹${rate.toFixed(2)} wallet में होना चाहिए।`});}
