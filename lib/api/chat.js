...
module.exports=async(req,res)=>{try{
 ...
 if(action==='request'||action==='call'){
  ...
  if(aid){
   ...
   let kundli=null;if(channel==='chat'){const kid=String(b.kundaliId||'').trim();if(kid){kundli=await kundliForUser(kid,u.id);if(!kundli)return json(res,403,{error:'Selected Kundli does not belong to this account'});}}
