...
async function decorate(c){
 if(!c)return null;
 const user=c.user_id?(await sbreq(`/rest/v1/user_profiles?id=eq.${esc(c.user_id)}&select=id,full_name,avatar_url`))[0]||null:null;
 const astrologer=c.astrologer_id?(await sbreq(`/rest/v1/astrologers?id=eq.${esc(c.astrologer_id)}&select=id,full_name,avatar_url`))[0]||null:null;
 const admin=c.admin_id?(await sbreq(`/rest/v1/admin_profiles?id=eq.${esc(c.admin_id)}&select=id,full_name,avatar_url`))[0]||null:null;
 return {...c,user,astrologer,admin};
}
...
async function listFor(u,p){
 ...
 let rows;
 try{rows=await sbreq(q)}catch(e){
   // PostgREST does not accept SQL expressions such as coalesce() in order=.
   // Fall back to a plain column order so chat list remains usable on all Supabase versions.
   const fallback=p?.role==='admin'?'/rest/v1/conversations?select=*&order=created_at.desc&limit=1000':`/rest/v1/conversations?or=(user_id.eq.${esc(u.id)},astrologer_id.eq.${esc(u.id)},admin_id.eq.${esc(u.id)})&select=*&order=created_at.desc&limit=300`;
   rows=await sbreq(fallback);
 }
 rows.sort((a,b)=>new Date(b.last_message_at||b.created_at||0)-new Date(a.last_message_at||a.created_at||0));
 rows=await Promise.all(rows.map(expire));return rows;
}
module.exports=async(req,res)=>{try{
 ...
 if(action==='request'||action==='call'){
  ...
  if(aid){
   ...
   // One persistent chat room per User + Astrologer pair.
   // Re-opening/requesting later must reuse the same conversation id so the
   // same realtime topic and complete chat history are preserved.
   const old=await sbreq(`/rest/v1/conversations?user_id=eq.${esc(u.id)}&astrologer_id=eq.${esc(aid)}&channel=eq.${esc(channel)}&select=*&order=created_at.asc&limit=1`);
   if(old[0]){
    const existing=await expire(old[0]);
    if(['requested','astrologer_accepted','accepted'].includes(existing.status)){
      return json(res,200,{conversation:await decorate(existing),existing:true,reusedRoom:true});
    }
    // Re-open the SAME room after a previous closed/missed/rejected session.
    // Do not create another conversation; all previous messages stay attached.
    const reopened=await sbreq(`/rest/v1/conversations?id=eq.${esc(existing.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({
      status:'requested',requested_at:new Date().toISOString(),astrologer_accepted_at:null,user_confirm_deadline:null,user_confirmed_at:null,accepted_at:null,closed_at:null,missed_by:null,astrologer_response_seconds:null,user_confirm_response_seconds:null,retention_until:null,fee_snapshot:Number(a.fee||0),discount_snapshot:Number(a.discount||0)
    })});
    return json(res,200,{conversation:await decorate(reopened[0]||{...existing,status:'requested'}),existing:true,reusedRoom:true,reopened:true});
   }
   ...
   const rows=await sbreq('/rest/v1/conversations',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});return json(res,201,{conversation:await decorate(rows[0]),existing:false,reusedRoom:false});
  }
  const admins=await sbreq('/rest/v1/admin_profiles?select=id,full_name,avatar_url&limit=1');const admin=admins[0];if(!admin)return json(res,503,{error:'No admin support account is available'});
