...
module.exports=async(req,res)=>{try{
  ...
  if(req.method==='GET'&&action==='pooja-bookings'){
    ...
    return json(res,200,{bookings:rows.map(b=>({...b,user:mu.get(String(b.user_id))||null,astrologer:ma.get(String(b.astrologer_id))||null,pooja:mp.get(String(b.pooja_id))||null}))});
  }
  ...
  if(req.method==='PATCH'){
    ...
    if(action==='pooja-booking'){
      ...
      const rows=await sbreq(`/rest/v1/pooja_bookings?id=eq.${esc(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status,admin_note:String(b.admin_note||''),updated_at:new Date().toISOString()})});
      await audit(u.id,'pooja_booking_'+status,{booking_id:id});return json(res,200,{booking:rows[0]||null});
    }
