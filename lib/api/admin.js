...
module.exports=async(req,res)=>{try{
  ...
  if(req.method==='GET'&&action==='users'){
    ...
    const userIds=rows.filter(x=>x.role==='user').map(x=>String(x.id));
    let wallets=[];
    if(userIds.length){
      try{wallets=await sbreq(`/rest/v1/wallets?user_id=in.(${userIds.map(esc).join(',')})&select=user_id,balance&limit=5000`)}catch(e){}
    }
    const walletMap=new Map(wallets.map(w=>[String(w.user_id),Number(w.balance||0)]));
    return json(res,200,{users:rows.map(x=>x.role==='user'?{...x,wallet_balance:walletMap.get(String(x.id))||0}:x)});
  }
  ...
  if(req.method==='PATCH'||req.method==='POST'){
    ...
    if(action==='user'){
      ...
      if('role' in b && String(b.role||'')!==actual) return json(res,400,{error:'Changing account role is not supported from this control. Use the correct account panel.'});
    ...
    if(action==='astrologer'){
      ...
      if('fee'in b)patch.fee=Math.max(0,Number(b.fee)||0);if('fee_per_minute'in b){patch.fee_per_minute=Math.max(0,Number(b.fee_per_minute)||0);patch.fee=patch.fee_per_minute;}if('discount'in b){const d=Number(b.discount);if(![0,25,50,75].includes(d))return json(res,400,{error:'Discount must be 0%, 25%, 50% or 75%'});patch.discount=d;}if(b.verified===true)patch.approved_at=new Date().toISOString();
