...
module.exports=async(req,res)=>{try{
  ...
  if(req.method==='PATCH'){
    ...
    if(action==='payment'){
      ...
      if(status==='approved'&&current.status!=='approved'&&current.purpose==='wallet_topup'){const w=await wallet(current.user_id);const next=Number(w.balance||0)+Number(current.amount||0);await sbreq(`/rest/v1/wallets?id=eq.${esc(w.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({balance:next,updated_at:new Date().toISOString()})});await sbreq('/rest/v1/wallet_transactions',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:current.user_id,amount:Number(current.amount||0),type:'credit',source:'admin_approved_payment',payment_id:current.id,description:'Wallet top-up approved by admin'})})}
