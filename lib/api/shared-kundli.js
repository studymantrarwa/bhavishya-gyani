...
module.exports=async(req,res)=>{try{
 ...
 let k=null;
 if(cid){const c=(await sbreq(`/rest/v1/conversations?id=eq.${esc(cid)}&select=*`))[0];if(!c)return json(res,404,{error:'Conversation not found'});const participant=c.user_id===u.id||c.astrologer_id===u.id;if(!participant)return json(res,403,{error:'Forbidden'});if(p?.role==='astrologer'&&c.astrologer_id===u.id&& !['requested','astrologer_accepted','accepted','closed'].includes(c.status))return json(res,403,{error:'Kundli access not allowed'});const owner=c.user_id;const rows=await sbreq(`/rest/v1/kundalis?user_id=eq.${esc(owner)}&select=*&order=created_at.desc&limit=1`);k=rows[0]||null;
 }else{
