...
module.exports=async(req,res)=>{
  try{
    ...
    const rows=await req('/rest/v1/profiles',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({id,full_name:name,email,phone:phone||null,role:'admin',updated_at:new Date().toISOString()})});
    await req('/rest/v1/admin_profiles',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({id,department:'Administration'})});
