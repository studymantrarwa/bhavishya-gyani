...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='POST'){
   ...
   const rows=await sbreq(`/rest/v1/conversations?id=eq.${esc(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:'accepted',fee_snapshot:currentRate,discount_snapshot:discount,astrologer_accepted_at:startAt,user_confirm_deadline:null,user_confirmed_at:null,accepted_at:startAt,chat_started_at:startAt,chat_budget_seconds:budget.total_seconds,billed_seconds:0,billed_amount:0})});
