...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'&&view==='earnings'){
   ...
   const sum=k=>rows.filter(x=>String(x.category||'service')===k).reduce((s,x)=>s+Number(x.amount||x.net_amount||0),0);
   ...
   return json(res,200,{total:netAmount,gross,commission,net:netAmount,varta:sum('varta'),affiliate:sum('affiliate'),reportAffiliate:sum('report_affiliate'),service:sum('service'),today:todayRows.reduce((s,x)=>s+Number(x.net_amount||x.amount||0),0),daily,payouts});
 }
