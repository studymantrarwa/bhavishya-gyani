...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='GET'&&!view){
   ...
   return json(res,200,{profile:p,astrologer:a,fee_per_minute:Number(a.fee_per_minute??a.fee??0),followers:Number(a.followers_count||0),rating:{average:avg,count:reviews.length},todayEarning:earn.reduce((s,x)=>s+Number(x.amount||0),0),connections:conv.length});
 }
 if(req.method==='GET'&&view==='earnings'){
   const rows=await safe(`/rest/v1/astrologer_earnings?astrologer_id=eq.${encodeURIComponent(u.id)}&earned_at=gte.${encodeURIComponent(monthStart())}&select=*`,[]);
   const sum=k=>rows.filter(x=>String(x.category||'service')===k).reduce((s,x)=>s+Number(x.net_amount??x.amount??0),0);
   const today=new Date().toISOString().slice(0,10);const todayRows=rows.filter(x=>String(x.earned_at||'').slice(0,10)===today);
   const daily={};for(const x of rows){const d=String(x.earned_at||'').slice(0,10);daily[d]=(daily[d]||0)+Number(x.amount||x.net_amount||0)}
   const gross=rows.reduce((s,x)=>s+Number(x.gross_amount??x.amount??0),0),commission=rows.reduce((s,x)=>s+Number(x.commission_amount??0),0),netAmount=rows.reduce((s,x)=>s+Number(x.net_amount??x.amount??0),0);
   const chatRows=rows.filter(x=>String(x.category||'')==='chat').map(x=>({chat_id:x.conversation_id||null,earned_at:x.earned_at||null,gross:Number(x.gross_amount??0),commission:Number(x.commission_amount??0),earning:Number(x.net_amount??x.amount??0),paid_minutes:Number(x.paid_minutes??0),paid_amount:Number(x.paid_amount??0)}));
   const payouts=await safe(`/rest/v1/payouts?astrologer_id=eq.${encodeURIComponent(u.id)}&select=*&order=period_start.desc&limit=24`,[]);
   return json(res,200,{total:netAmount,gross,commission,net:netAmount,varta:sum('varta'),affiliate:sum('affiliate'),reportAffiliate:sum('report_affiliate'),service:sum('service'),today:todayRows.reduce((s,x)=>s+Number(x.net_amount??x.amount??0),0),daily,payouts,chats:chatRows});
 }
