...
function localBridge(input){
  ...
  const lines=out.split(/\r?\n/).filter(Boolean); const data=JSON.parse(lines[lines.length-1]);
...
function productionUrl(){
  if(process.env.EPHEMERIS_URL)return String(process.env.EPHEMERIS_URL).replace(/\/$/,'');
  if(process.env.VERCEL_URL)return `https://${process.env.VERCEL_URL}/api/ephemeris`;
  if(process.env.VERCEL_PROJECT_PRODUCTION_URL)return `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}/api/ephemeris`;
  throw new Error('Vercel deployment URL is unavailable. Set EPHEMERIS_URL if using a custom backend URL.');
}
async function remoteBridge(input){
  const base=productionUrl(); const secret=process.env.EPHEMERIS_SECRET||'';
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),25000);
  try{
    const r=await fetch(base,{method:'POST',headers:{'Content-Type':'application/json','x-ephemeris-secret':secret},body:JSON.stringify(input),signal:ctrl.signal,cache:'no-store'});
    ...
    if(e.name==='AbortError')throw new Error('Swiss Ephemeris service timed out after 25 seconds');
    throw e;
...
async function calculateEphemeris(input){
  ...
  try{return isVercel?await remoteBridge(input):localBridge(input)}
    ...
    if(isVercel){const e=new Error(`Swiss Ephemeris unavailable in production: ${err?.message||String(err)}`);e.code='EPHEMERIS_REQUIRED';throw e;}
